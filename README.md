...
- **Live code rain**, generated by the addon instead of played from a file:
  no video, no download, no network, and it starts instantly
...
1. `render/glyphs.py` describes 34 katakana-like glyphs as line segments and
   rasterises them, mirrored as well, into small coverage maps. A font would
   not do: Kodi resolves fonts against the active skin, and the characters the
   films use are not part of it.
...
frame rate with no per-frame work in Python at all. The textures depend only on
the design resolution, not on the screen: the window scales them, which keeps
the glyphs square from 720p up to ultra-wide. Generating all 64 of them takes
well under a second and only happens once -- they are cached in the addon's
profile folder (`addon_data/screensaver.matrix/rain`) and reused from there.
...
resources/
  ...
  lib/
    ...
    render/
      glyphs.py              the glyph set, described as strokes and rasterised
      rain.py                builds and caches the column textures
      png.py                 minimal PNG encoder for those textures
    download/
