...
| --- | --- | --- | --- |
| 3D | matrixcode | Dense and quick, with short drops | perspective |
...
and Bugs -- and a stack of flat images cannot do that, so they are drawn flat
with everything else they configure intact. Nightmare leans on its thunder to
light the screen up, so without it the variant stays dark; that is what its raw
output looks like.
